...
import { register } from '@/routes/webauthn';
import { deleteMethod } from '@/routes/passkeys';
import { index as passkeys } from '@/routes/passkeys';
...
export default function Passkey({passkeys} : { passkeys: Passkey[] }) {
  ...
  const submit: FormEventHandler = async (e) => {
    ...
    try {
      const { success, error } = await Webpass.attest({path: registerChallenge.url(), findCsrfToken: true}, {path: register.url(), findCsrfToken: true, body: {alias: alias}})
      ...
      if (success) {
        ...
        toast.error((error as {message: string})?.message || 'Failed to create passkey');
      }
    } catch (err: unknown) {
      console.error(err);
